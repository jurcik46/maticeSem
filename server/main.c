...
void cofatorMatrix(int rc, struct Fraction matrix[N][N], struct Fraction tempMatrix[N][N], int p, int q)
{
    int i = 0, j = 0;
...
struct Fraction determinantMatrix(uint32_t rc, struct Fraction matrix[N][N])
{
    printf("----------------------------------RC %d------------------------\n", rc);
    ...
    struct Fraction tempMatrix[N][N];
...
int main(int argc, char **argv)
{
    ...
    struct Fraction k = determinantMatrix(N, o);
